...
$pdf->SetHeaderData('','','Delivery Sheet',$currentDate.' '.$currentTime, array(0,64,255), array(0,64,128));
...
FROM invoice
    ...
    $html.='
<table cellspacing="5" style="padding:2px">
<tr>
<td >
...
';
    $q="select * from application where application_invoice_no=".$invoiceno;
    $r=mysqli_query($connection,$q);
    $rw=mysqli_fetch_assoc($r);
$pdf->writeHTMLCell(0, 0, '', '', $html, true); 
$pdf->ln(40);
$pdf->setfont('times','B',12);
$pdf->cell(120,0,'Passport and Original Document Delivery Note',0,1,'L');
$pdf->cell(120,0,'Invoice No :'.$row1['invoice_no'],0,0,'L');
$pdf->cell(60,0,'Delivery By :'.$rw['application_deliveredBy'],0,1,'L'); 
$pdf->cell(120,0,'',0,0,'L');
$pdf->cell(60,0,'Delivery Date :'.$rw['application_deliveredDate'],0,1,'L'); 
$pdf->setfont('times','',10);
$pdf->ln();
...
<th align="center">Embassy Ref. No.</th>
</tr>';
...
where invoice.invoice_no='.$invoiceno;
    ...
    while($row2=mysqli_fetch_assoc($result)) {
        $fullname=strtoupper($row2['applicant_firstname'].' '.$row2['applicant_lastname']);
 $html.='   
<tr>
...
<td align="center">'.$row2['application_referenceno'].'</td>
</tr>
';$i++;
   $count++; }
...
';
 //   $pdf->writeHTMLcell(0, 0, '', '', $html, true); 
   ...
   $message="List of Relative Documents: ".$row1['related_documents'];
    $pdf->multicell(0,0,$message,1,'L',0);
   ...
   $message="List of Supportive Documents: ".$row1['supportive_documents'];
    ...
    $message="Please Cancel Original documents where already returned with other Passports in the group";
     $pdf->multicell(0,0,$message,0,'C',0);
    $pdf->ln(10);
    $pdf->writeHTML('<hr>', true, false, false, false, '');
    $pdf->cell(120,0,'Total Number of Passport : '.$count,0,0,'',0,'',0,false,'T','T');
     $pdf->cell(40,0,'Check and Delivered by: ',0,0,'',0,'',0,false,'T','T');
    $pdf->ln(10);
    $pdf->setfont('times','',11);
    $pdf->setCellPaddings(5,5,5,5);
    $pdf->cell(0,0,'Name and Signature of the Applicant Recieving the Passport and Documents from RTS-KBL',1,0,'C');
